...
const GameBoard = () => {
   ...
   const WIDTH = 500;
   const [snakebar, setsnakebar] = useState<posType[]>([{ x: 0, y: 0 }])
   ...
   return (
      <div style={{ position: "relative", width: WIDTH, aspectRatio: "1 / 1", content: "", background: colors.Beige }}>
         {snakebar.map((i, idx) => <span key={`snake_bar_${idx}`} id="snakebar" style={{ left: i.x, top: i.y }} />)}
         <span id="score" style={{ left: 60, top: 60 }} className="animate-pulse" />
      </div>
   )
